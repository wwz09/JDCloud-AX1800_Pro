...
GOLANG_REPO="hkcname.gh-proxy.com/https://github.com/sbwml/packages_lang_golang"
GOLANG_BRANCH="25.x"
THEME_SET="argon"
...
update_feeds() {
    ...
    if ! grep -q "small-package" "$BUILD_DIR/$FEEDS_CONF"; then
        ...
        echo "src-git small8 hkcname.gh-proxy.com/https://github.com/kenzok8/small-package" >>"$BUILD_DIR/$FEEDS_CONF"
    fi
    ...
    #     [ -z "$(tail -c 1 "$BUILD_DIR/$FEEDS_CONF")" ] || echo "" >>"$BUILD_DIR/$FEEDS_CONF"
    #     echo "src-git nss_packages hkcname.gh-proxy.com/https://github.com/LiBwrt/nss-packages.git" >>"$BUILD_DIR/$FEEDS_CONF"
    # fi
...
update_ath11k_fw() {
    ...
    local url="hkcname.gh-proxy.com/https://raw.githubusercontent.com/VIKINGYFY/immortalwrt/refs/heads/main/package/firmware/ath11k-firmware/Makefile"
...
add_ax6600_led() {
    ...
    local repo_url="hkcname.gh-proxy.com/https://github.com/NONGFAH/luci-app-athena-led.git"
...
update_tcping() {
    ...
    local url="hkcname.gh-proxy.com/https://raw.githubusercontent.com/xiaorouji/openwrt-passwall-packages/refs/heads/main/tcping/Makefile"
...
update_homeproxy() {
    local repo_url="hkcname.gh-proxy.com/https://github.com/immortalwrt/homeproxy.git"
...
update_package() {
    ...
    if [ -f "$mk_path" ]; then
        ...
        local PKG_VER
        if ! PKG_VER=$(curl -fsSL "hkcname.gh-proxy.com/https://api.github.com/repos/$PKG_REPO/$branch" | jq -r '.[0] | .tag_name // .name'); then
            echo "错误：从 hkcname.gh-proxy.com/https://api.github.com/repos/$PKG_REPO/$branch 获取版本信息失败" >&2
            return 1
        fi
        if [ -n "$3" ]; then
            PKG_VER="$3"
        ...
        local COMMIT_SHA
        if ! COMMIT_SHA=$(curl -fsSL "hkcname.gh-proxy.com/https://api.github.com/repos/$PKG_REPO/tags" | jq -r '.[] | select(.name=="'$PKG_VER'") | .commit.sha' | cut -c1-7); then
            echo "错误：从 hkcname.gh-proxy.com/https://api.github.com/repos/$PKG_REPO/tags 获取提交哈希失败" >&2
            return 1
        fi
...
fix_quickstart() {
    ...
    local url="hkcname.gh-proxy.com/https://gist.githubusercontent.com/puteulanus/1c180fae6bccd25e57eb6d30b7aa28aa/raw/istore_backend.lua"
...
add_timecontrol() {
    ...
    local repo_url="hkcname.gh-proxy.com/https://github.com/sirpdboy/luci-app-timecontrol.git"
    # 删除旧的目录（如果存在）
...
add_gecoosac() {
    ...
    local repo_url="hkcname.gh-proxy.com/https://github.com/lwb1978/openwrt-gecoosac.git"
    # 删除旧的目录（如果存在）
...
update_geoip() {
    ...
    if [ -d "${geodata_path%/*}" ] && [ -f "$geodata_path" ]; then
        ...
        if [ -n "$GEOIP_VER" ]; then
            local base_url="hkcname.gh-proxy.com/https://github.com/v2fly/geoip/releases/download/${GEOIP_VER}"
...
update_smartdns() {
    # smartdns 仓库地址
    local SMARTDNS_REPO="hkcname.gh-proxy.com/https://github.com/pymumu/openwrt-smartdns.git"
    local SMARTDNS_DIR="$BUILD_DIR/feeds/packages/net/smartdns"
    # luci-app-smartdns 仓库地址
    local LUCI_APP_SMARTDNS_REPO="hkcname.gh-proxy.com/https://github.com/pymumu/luci-app-smartdns.git"
...
update_diskman() {
    ...
    local repo_url="hkcname.gh-proxy.com/https://github.com/lisaac/luci-app-diskman.git"
    if [ -d "$path" ]; then
        echo "正在更新 diskman..."
...
add_quickfile() {
    local repo_url="hkcname.gh-proxy.com/https://github.com/sbwml/luci-app-quickfile.git"
...
update_argon() {
    local repo_url="hkcname.gh-proxy.com/https://github.com/ZqinKing/luci-theme-argon.git"
